...
/**
 * Utility class to send commands to the server. The client sends commands to the server in order to control the creature. Each command corresponds to an action that the creature must perform or a
 * request to change a specific attribute of the creature.
 *
 * @author ecalhau
 */
public class CommandUtility {
    ...
    /**
     * Command to create a visual reference (e.g. arrow) at a specific point within the server environment and expressed in Cartesian coordinate system (x,y). Each point within the server environment
     * is called a "waypoint"
     *
     * @param x abscissa of the point
     * @param y ordinate of the point
     * @return server response: Point (x, y) where the waypoint was created. Format: "x y"
     * @throws CommandExecException An exception is thrown in case of missing or invalid parameter
     */
    ...
    /**
     * Command to set the simulation game environment dimension: width, height and the texture for the simulation floor.
     *
    ...
    /**
     * Command to delete the visual reference (arrow) which indicates a location (waypoint) within the server environment.
     *
     * @param x x abscissa of the waypoint
     * @param y y ordinate of the waypoint
     * @return report from server: position of the deleted waypoint. Format "x y"
     * @throws CommandExecException An exception is thrown in case of missing or invalid parameter
     */
    ...
    /**
     * Command to get the next creature destination.
     *
     * @return X and Y coordinates for the new position.
     * @throws CommandExecException An exception is thrown in case of missing or invalid parameter
     */
    public static synchronized StringTokenizer sendGetDestination() throws CommandExecException {
        String controlMessage = "getdestination";
        return sendCmdAndGetResponse(controlMessage);
    }

    /**
     * Command to set the next creature destination.
     *
     * @param X and Y coordinates for the new position.
     * @throws CommandExecException An exception is thrown in case of missing or invalid parameter
     */
    public static synchronized StringTokenizer sendSetDestination(int x, int y) throws CommandExecException {
        String controlMessage = "setdestination " + x + " " + y;
        return sendCmdAndGetResponse(controlMessage);
    }
     ...
     *
     * The angle of the turn is evaluated according to the direction established between the points: robot location and (X,Y) position (waypoint). The creature speed (i.e. linear velocity) is the
     * arithmetic mean of Vr and Vl.
     *
    ...
    /**
     * Set the rotation around the creature's axis of symmetry. The creature's movement is changed in order to make it faces towards a certain direction. The creature speed (i.e. linear velocity) is
     * the arithmetic mean of Vr and Vl. The angle must be in rad.
     *
     * @param vr linear velocity of the right wheel
     * @param vl linear velocity of the left wheel
    ...
    /**
     * This command makes the creature turn a specific angle. The angle of the turn is derived from the linear velocity of the wheels (Vr-right Vl-left). The velocity of the creature's overall
     * movement corresponds to the speed parameter.
     *
     * Note: The final pitch of the robot depends on its original orientation (pitch). HINTs: +~30deg: Vl= 11 and Vr= 0.6; +~45deg: Vl= 16 and Vr= 0.2; +~60deg: Vl= 22 and Vr= 1;+~90deg: Vl= 32 and
     * Vr= 0.6; +~180deg: Vl= 63 and Vr= 0.2; +~270deg: Vl= 95 and Vr= 0.8.
     *
     *
     * @param robotID sequence number assigned to each object once it is created.
    ...
    /**
     * Randomly generates a new leaflet. A leaflet is a list of crystals to be collected and delivered at the DeliverySpot.
     *
     * @return the leaflets (3 in total). Format: "Leaflet1 Leaflet2 Leaflet3" Leaflet format: "[Color TotalNumber NumberCollected] Payment Example: White 2 0 Yellow 1 0 6 White 1 0 Magenta 2 0 5 Blue
     * 1 0 Red 1 0 Green 1 0 24 Leaflet1: 2 white jewels and 1 yellow jewel. None already collected. This leaflet increases the score in 6 points Leaflet2: 1 white jewel and 2 magenta jewels. None
     * already collected. This leaflet increases the score in 5 points Leaflet3: 1 Blue, 1 Red and 1 green jewel. None already collected. This leaflet increases the score in 24 points
     * @throws CommandExecException An exception is thrown in case of missing or invalid parameter
     */
    ...
    /**
     * The leaflet is delivered at the DeliverySpot. All the crystals specified in the leaflet were collected and the creature is at the DeliverySpot.
     *
     * @param robotID
     * @param leafletID
     *
    ...
    /**
     * The creature grasp and put an object in its sack. The object may be: food or crystal.
     *
     * @param thingName the unique name of the Thing. Example: Jewel_1363904023537
     * @return report from server with the name of the grasped Thing. Format: "ThingName".
     * @throws CommandExecException An exception is thrown in case of missing or invalid parameter
     */
     ...
     *
     * @param thingName the unique name of the Thing. Example: Jewel_1363904023537
     * @return report from server with the name of the Thing which has been eaten. Format: "ThingName".
     * @throws CommandExecException An exception is thrown in case of missing or invalid parameter
     */
     ...
     * @return report from server with the point (x,y) where the Thing has been dropped. Format: "x y".
     * @throws CommandExecException An exception is thrown in case of missing or invalid parameter
     */
     ...
     *
     * @param thingName the unique name of the Thing. Example: Jewel_1363904023537
     * @return report from server with the name of the Thing which has been hidden. Format: "ThingName".
     * @throws CommandExecException An exception is thrown in case of missing or invalid parameter
     */
     ...
     *
     * @param thingName the unique name of the Thing. Example: Jewel_1363904023537
     * @return report from server with the name of the Thing which has been "unearthed". Format: "ThingName".
     * @throws CommandExecException An exception is thrown in case of missing or invalid parameter
     */
     ...
     *
     * @param robotID an integer that is a sequence number assigned to each object once it is created. It specifies a creature.
    ...
    /**
     * Check if the specified creature has been already created on the server at a certain position. If it already exists but with different pitch, the pitch is changed.
     *
    ...
    /**
     * A name that identifies the client. Examples: RobotMind_0, RobotMind_1 etc.
     *
     * @param robotID sequence number assigned to each object once it is created.
     ...
     *
     * Note: Currently, there are only 2 cameras available on the server. The right camera is attached to even robotIDs and the left camera, to the odd robotIDs.
     *
     * @param robotID sequence number assigned to each object once it is created.
     * @return report from server with the "creature index" indicating the current owner of a camera. Even indexes are attached to right camera and odd indexes to the left one. Example: camera 0 0
     * @throws CommandExecException An exception is thrown in case of missing or invalid parameter
     */
    ...
    /**
     * Get the values of the simulation parameters: width, height, xDS, yDS width and height of the environment of the game (simulation) (xDS, yDS) location of the DeliverySpot.
     *
     ...
     *
     * @return a string indicating an approximation to the total amount of memory currently available for future allocated objects, measured in bytes.
     * @throws CommandExecException An exception is thrown in case of missing or invalid parameter
     */
     ...
     *
     * @return a string indicating the difference, measured in milliseconds, between the current time and midnight, January 1, 1970 UTC.
     * @throws CommandExecException An exception is thrown in case of missing or invalid parameter
     */
     ...
     *
     * @param robotID sequence number assigned to each object once it is created.
     * @return the creature position in the format: "X Y Pitch" Example: 287.0 391.0 0.0
     * @throws CommandExecException An exception is thrown in case of missing or invalid parameter
     */
     ...
     *
     * @param robotID sequence number assigned to each object once it is created.
     * @return creature's speed and fuel values in the format: "speed fuel" Example: 1.0 1000
     * @throws CommandExecException An exception is thrown in case of missing or invalid parameter
     */
     ...
     *
     * @param robotID sequence number assigned to each object once it is created.
     * @return Return the creature's bag content: Total number of Food; Total number of Crystals; Number of PerishableFood; Number of Non-PerishableFood; Number of crystals each color in the sequence:
    ...
    /**
     * Returns the affordances of a Thing. The server respond with the list of affordances code. Codes: Affordance__VIEWABLE = 30; Affordance__HIDEABLE = 31; Affordance__UNHIDEABLE = 32;
     * Affordance__GRASPABLE = 33; Affordance__EATABLE = 34; Affordance__PUTINBAGABLE = 35;//sth that can be put in a bag; Affordance__OPENABLE = 36; //sth that can be opened (eg. a cage);
     * Affordance__CLOSEABLE = 37;//sth than be closed (eg. a cage); Affordance__INSERTABLE = 38;//sth than can contain another thing (which had been inserted into this sth)[eg. a container];
     ...
     * @return list of affordance codes. Example: 30 31 32 Meaning: Affordance__VIEWABLE Affordance__HIDEABLE Affordance__UNHIDEABLE
     * @throws CommandExecException An exception is thrown in case of missing or invalid parameter
     */
     ...
     *
     * @return environment dimension in the format: "width height". Example: 800 600
     * @throws CommandExecException An exception is thrown in case of missing or invalid parameter
     */
    ...
    /**
     * Returns the number of creatures and other Things currently at the simulation environment.
     *
     * @return server response with the number of entities. Format: "NumCreatures NumOtherThings"
     * @throws CommandExecException An exception is thrown in case of missing or invalid parameter
     */
    ...
    /**
     * Returns the creatures and other Things currently at the simulation environment.
     *
     * @return server response with the entities. Format: "numberOfEntities ThingName1 category isOccluded X1 Y1 X2 Y2 pitch colorIndex ThingName2..." Example: 3 Creature_1364966079239 0 0 275.0 275.0
     * 373.0 373.0 0 Jewel_1364966084869 3 0 552.0 552.0 355.0 355.0 1 NPFood_1364966088030 22 0 469.0 469.0 311.0 311.0 1
     *
     * @throws CommandExecException An exception is thrown in case of missing or invalid parameter
     */
     ...
     *
     * @param robotID sequence number assigned to each object once it is
     * @return server response with the number of entities. Format: "ThingName1 category isOccluded X1 X2 Y1 Y2 pitch hardness energy shininess colorName"
     * @throws CommandExecException An exception is thrown in case of missing or invalid parameter
     */
     ...
     * @return server successful response with creature position. Format: "CreatureIndex CreatureName X Y Pitch"
     * @throws CommandExecException An exception is thrown in case of missing or invalid parameter
     */
     ...
     * @return server successful response with cage position. Format: "CageName X Y"
     * @throws CommandExecException An exception is thrown in case of missing or invalid parameter
     */
     ...
     * @return server successful response with food position. Format: "FoodName X Y"
     * @throws CommandExecException An exception is thrown in case of missing or invalid parameter
     */
     ...
     * @param X_Y text with the list of location where a new food must be created
     * @return server successful response with food names and positions. Format: "FoodName0 X Y FoodName1 X Y ..."
     * @throws CommandExecException An exception is thrown in case of missing or invalid parameter
     */
     ...
     * @param X_Y text with the list of location where a new jewel must be created
     * @return server successful response with jewel names and positions. Format: "JewelName0 X Y JewelName1 X Y ..."
     * @throws CommandExecException An exception is thrown in case of missing or invalid parameter
     */
     ...
     * @return server successful response with jewel position. Format: "JewelName X Y"
     * @throws CommandExecException An exception is thrown in case of missing or invalid parameter
     */
     ...
     * @return server successful response with brick position. Format: "BrickName X Y"
     * @throws CommandExecException An exception is thrown in case of missing or invalid parameter
     */
    ...
    /**
     * The creature state refers to its location and camera content (list of objects that have been captured).
     *
     * @param robotNameID name that uniquely identifies the creature in the system
     *
     * @return list of creature's attributes and the objects that have been captured by the camera (creature's visual system).
     * @throws CommandExecException An exception is thrown in case of missing or invalid parameter
     */
     ...
     * @param thingID sequence number assigned to each creature and to other entities (different pools)
    ...
    /**
     * Creates a creature with its initial attribute values: either default values or those got from a creature already present in the server.
     *
     * @param indexID
     * @param nameID
     * @return
    ...
    /**
     * Get information regarding the creature state and the content of its visual system (camera) according to a protocol established with the server. The information is used to create the list of
     * objects (Thing) that are manipulated in the "mind".
     *
     * @param unique ID of the Creature (e.g. Creature_1364966079239)
     * @return the creature state that encapsulates its attributes such as position, color, pitch, energy level etc
     * @throws CommandExecException An exception is thrown in case of missing or invalid parameter
     */
